...
### 2.1 设计语言：Apple Notes 原生感

| 设计要素 | 方案 |
|----------|------|
| **整体风格** | 苹果备忘录风格 — 温暖、亲和、沉浸式写作体验 |
| **底色** | 编辑区暖米色 (`#FEF9F0` 或类似)，侧边栏略深 |
| **侧边栏** | 毛玻璃效果（`backdrop-blur`），半透明底色 |
| **圆角** | 大圆角（12-16px），macOS Ventura 风格 |
| **阴影** | 极柔和，低透明度，营造浮层感 |
| **字体** | 系统默认（macOS: SF Pro, Windows: Segoe UI）+ 等宽代码字体 JetBrains Mono |
| **动效** | 平滑过渡（200-300ms ease-out），无突兀动画 |
| **窗口** | Tauri 无边框 + 自定义标题栏，支持窗口拖拽 |
| **色调** | 亮色暖色调为主，暗色深灰 + 低对比暖色 |
| **间距** | 宽松舒适，大留白，内容区 max-width 控制阅读宽度 |
...
:root {
  /* 亮色主题 */
  --bg-window:        #f5f5f7;      /* 窗口底色（类似 macOS 窗口） */
  --bg-sidebar:       rgba(245,245,247,0.72); /* 侧边栏毛玻璃 */
  --bg-editor:        #fef9f0;      /* 编辑区暖米色 */
  --bg-preview:       #ffffff;      /* 预览区纯白 */
  --bg-toolbar:       rgba(255,255,255,0.64);
  
  --text-primary:     #1d1d1f;      /* 主文字 */
  --text-secondary:   #86868b;      /* 次要文字 */
  --text-muted:       #aeaeb2;      /* 占位/禁用文字 */
  ...
  --font-sans:        -apple-system, 'SF Pro Display', 'Segoe UI', sans-serif;
...
### 5.0 首页 + 主界面布局（Apple Notes 原生感）

采用 IDE 风格三栏布局，侧边栏常驻 + 毛玻璃效果，无文件时中央显示欢迎引导。

**亮色主题效果**：侧边栏半透明毛玻璃，编辑器暖米色底，整体柔和无强烈对比。
...
- **无边框窗口**（Tauri）：自定义标题栏，支持窗口拖拽，macOS 红绿灯按钮融入设计
- **左侧边栏**（~240px）：毛玻璃效果（`backdrop-blur: 20px`），半透明底，可折叠
  - 文件树 + 最近文件列表
  - 底部账户/设置入口
- **中央编辑区**：暖米色底色，TipTap 编辑器，无文件时显示欢迎引导
  - 欢迎引导：Logo + 新建/打开按钮 + 最近文件
- **右侧 AI 面板**（~320px）：毛玻璃效果，默认收起，点击工具栏 🤖 展开
- **底部状态栏**：半透明，显示视图模式、文件名、修改状态、字数
...
- 加入 Tauri 框架，配置无边框窗口（`decorations: false`）
- 自定义标题栏组件（macOS 红绿灯适配）
- 配置 Tailwind CSS + shadcn/ui + Radix
- 实现 CSS 变量体系（亮色/暗色主题色板）
- 建立 Zustand store 骨架
- 实现首页 UI（欢迎引导 + 侧边栏毛玻璃）
